...
def detect(rows: list[dict]) -> list[dict]:
    ...
    add("title_too_short", "Low",
        ...
        "Titles under 30 characters are too short for good search visibility.")
    ...
    add("meta_description_too_long", "Low",
        ...
        "Meta descriptions over 155 characters will be truncated in search results.")
    ...
    add("thin_content", "Low",
        ...
        "Indexable pages with fewer than 200 words — may be seen as low quality.")

    # --- Non-indexable but linked ---
    add("non_indexable_but_linked", "Medium",
        [r["Address"] for r in html
         if not indexable(r) and _int(r.get("Inlinks")) > 0],
        "Non-indexable pages that still receive internal links, wasting crawl budget.")
